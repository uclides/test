...
CREATE TABLE `device` (
  ...
  CONSTRAINT `id_bat` FOREIGN KEY (`id_bat`) REFERENCES `battery` (`id_bat`),
...
CREATE TABLE `battery` (
  ...
  PRIMARY KEY (`id_bat`),
  KEY `id_dev_bat` (`id_dev`),
  CONSTRAINT `id_dev_bat` FOREIGN KEY (`id_dev`) REFERENCES `device` (`id_device`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `device-format` (
  ...
  KEY `id_for_dev` (`id_dev`),
  KEY `id_for_for` (`id_for`),
  CONSTRAINT `id_for_dev` FOREIGN KEY (`id_dev`) REFERENCES `device` (`id_device`),
...
CREATE TABLE `image` (
  ...
  `id_dev` int(11) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id_img`),
  KEY `id_img_dev` (`id_dev`),
  CONSTRAINT `id_img_dev` FOREIGN KEY (`id_dev`) REFERENCES `device` (`id_device`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
